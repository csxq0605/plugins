...
def email_setup(
    ...
    password: str,
    name: str = "",
    imap_server: str = "mail.stu.pku.edu.cn",
    imap_port: int = 993,
    smtp_server: str = "mail.stu.pku.edu.cn",
    smtp_port: int = 465
) -> Dict[str, Any]:
    """
    配置邮件账户
    ...
    try:
        ...
        return {
            "success": False,
            "error": f"IMAP 登录失败: {str(e)}"
        }
    except smtplib.SMTPAuthenticationError as e:
        return {
            "success": False,
            "error": f"SMTP 登录失败: {str(e)}"
        }
...
def email_get_config() -> Dict[str, Any]:
    ...
    if not config.get("email"):
        return {
            "success": False,
            "error": "未配置邮件账户，请先使用 email_setup 配置"
        }
    ...
    return {
        ...
        "config": {
            ...
            "password": "***已配置***" if config.get("password") else "未配置"
        }
    }
